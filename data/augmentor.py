...
from data.robotcar_sdk.python.transform import euler_to_so3
...
class Augmentor(object):
    def __init__(self):
        ...
        self.rot_rollmax= 10
        self.p_rot_pitch= 0
        self.rot_pitchmin= -10
        self.rot_pitchmax= 10
        self.p_rot_yaw= 1
        self.rot_yawmin= -180
        self.rot_yawmax= 180
    ...
    def doAugmentation(self, pointcloud):
        ...
        if rand < self.p_rot_roll:
            rot_roll = random.uniform(self.rot_rollmin, self.rot_rollmax)
        else:
            rot_roll = 0
        rand = random.uniform(0, 1)
        if rand < self.p_rot_pitch:
            rot_pitch = random.uniform(self.rot_pitchmin, self.rot_pitchmax)
        else:
            rot_pitch = 0
        rand = random.uniform(0, 1)
        if rand < self.p_rot_yaw:
            rot_yaw = random.uniform(self.rot_yawmin, self.rot_yawmax)
        else:
            rot_yaw = 0
        ...
        # translation
        rand = random.uniform(0, 1)
        if rand < self.p_transx:
            trans_x = random.uniform(self.trans_xmin, self.trans_xmax)
        else:
            trans_x = 0
        rand = random.uniform(0, 1)
        if rand < self.p_transy:
            trans_y = random.uniform(self.trans_ymin, self.trans_ymax)
        else:
            trans_y = 0
        rand = random.uniform(0, 1)
        if rand < self.p_transz:
            trans_z = random.uniform(self.trans_zmin, self.trans_zmax)
        else:
            trans_z = 0
        pointcloud, translation = self.translation(pointcloud, trans_x, trans_y, trans_z)

        return pointcloud, rotation, translation